...
using InstructorApp.Authentication;
using InstructorApp.Entities;
using InstructorApp.EntityFrameworkCore;

namespace InstructorApp.Web.Host.Controllers
{
        ...
        public ActionResult Authenticate([FromBody] AuthenticateModel model)
        {
            try
            {
                ...
                if (user.PasswordHash != null && user.PasswordHash.StartsWith("PBKDF2$"))
                    pwOk = InstructorApp.Users.PasswordHelper.VerifyPbkdf2(model.Password, user.PasswordHash);
                else
