...
namespace InstructorApp.EntityFrameworkCore.Seed
{
    /// <summary>
    ...
    public class DefaultUserSeeder
    {
        ...
        public void Seed()
        {
            ...
            try
            {
                ...
                Console.WriteLine("[Seed] Step 5: Custom role 'Instructor'");
                var customRole_0 = _roleRepo.GetAll().FirstOrDefault(r => r.Name == "Instructor");
                if (customRole_0 == null)
                {
                    customRole_0 = new AppRole { Name = "Instructor", DisplayName = "Hoca", Description = "Spor salonu hocası — tüm üyeleri görebilir, ders kaydı oluşturur, katılım ve seans takibi yapar", IsSystem = false, IsActive = true };
                    customRole_0.Id = _roleRepo.InsertAndGetId(customRole_0);
                    Console.WriteLine($"[Seed] Created custom role 'Instructor' (id={customRole_0.Id})");
                }
                var customPerms_0 = new[] { "Member.Read", "Member.Create", "Member.Update", "MemberInstructor.Create", "MemberInstructor.Read", "MemberInstructor.Update", "Lesson.Create", "Lesson.Read", "Lesson.Update", "Lesson.Delete", "LessonAttendance.Create", "LessonAttendance.Read", "LessonAttendance.Update", "Payment.Create", "Payment.Read" };
                ...
                {
                    if (!allPermSet_0.Contains(pn)) { Console.WriteLine($"[Seed] Skipping unknown permission '{pn}' for role 'Instructor'"); continue; }
