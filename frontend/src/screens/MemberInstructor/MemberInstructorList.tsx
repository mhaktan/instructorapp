...
import { MemberInstructorCreate } from './MemberInstructorCreate';
import { MemberInstructorEdit } from './MemberInstructorEdit';
...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'assignedDate', header: 'Assigned Date', sortable: true, filterType: 'datepicker', html: (row: Record<string, unknown>) => row.assignedDate ? new Date(String(row.assignedDate)).toLocaleDateString() : '—' },
...
// ---------------------------------------------------------------------------
// MemberInstructorList
// ---------------------------------------------------------------------------

export const MemberInstructorList: React.FC = () => {
  const list = useListQuery<MemberInstructorRecord>({ resource: 'MemberInstructor' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<MemberInstructorRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<MemberInstructorRecord[]>([]);

  const del = useDeleteMutation('MemberInstructor');
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as MemberInstructorRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create MemberInstructor" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <MemberInstructorCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <MemberInstructorEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
