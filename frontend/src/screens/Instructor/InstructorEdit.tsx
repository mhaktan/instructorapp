...
interface InstructorEditProps {
  record: InstructorRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const InstructorEdit: React.FC<InstructorEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<InstructorRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<InstructorRecord>) =>
      dataProvider.update('Instructor', record!.id, values),
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit Instructor</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                ...
                <div>
                  <TkInput mode="text" label="Specialization" value={String(form.specialization ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('specialization', v))(e.detail)} />
                </div>
                <div>
