...
interface LessonAttendanceEditProps {
  record: LessonAttendanceRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const LessonAttendanceEdit: React.FC<LessonAttendanceEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<LessonAttendanceRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<LessonAttendanceRecord>) =>
      dataProvider.update('LessonAttendance', record!.id, values),
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit LessonAttendance</h2>
